...
let allCaps = runners.map(function(runner, index) {
  return runners[index].first_name.toUpperCase();
});
...
let largeShirts = runners.filter(function(runner, index){
    return runners[index].shirt_size === 'L';
});
...
let ticketPriceTotal = runners.reduce(function(accumulator, currentValue, currentIndex, sourceArray) {
    return accumulator + runners[currentIndex].donation;
});
